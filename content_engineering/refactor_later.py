...
if __name__ == "__main__":
    comparator = HTMLComparator(directory="html_from_sf")
    ...
    for key, result in results.items():
        ...
        print(f"Longer document: {result['longer_type']}")
        print(f"Number of chunks: {len(result['chunks'])}")
        print(f"Embedding shape: {result['embedding'].shape if result['embedding'] is not None else 'N/A'}")
        print("-" * 50)